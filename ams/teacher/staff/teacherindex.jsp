    ...
    <body>
        <%=session.getAttribute("tid")%>
            <div id="main_div">
                ...
                <div class="index-div d-flex align-items-center justify-content-center" style="height: 250px;">
                    <div class="login-div ">
            ...
            <script>
                var s1 =<%="'" + (String)session.getAttribute("tid") + "'" %>
                    async function fetchText() {
                        let response = await fetch('subjectview?tid=' + s1);
                ...
                fetchText().then((data) => {
                    ...
                    for (let x = 0; x < data.length; x++) {
                        s += "<tr><td>" + data[x][0] + "</td><td>" + data[x][1] + "</td><td><button class=\"btn btn-success\" onclick='location=\"attendance.jsp?sub_id=" + data[x][0] + "\"''>Attendance</button><button class=\"btn btn-info\" onclick='loadprevious(\"" + data[x][0] + "\")'>Previous Attendance</button></td></tr>";
                    }
                ...
                function loadprevious(subject_id) {
                    ...
                    async function fetchText() {
                        let response = await fetch('previousattendance?subject_id=' + subject_id);
                        let data = await response.json();
                        return data;
                    }
                    fetchText().then((data) => {
                        var s = "<h2>Previous Attendance</h2><table><tr><th>Date</th><th>Student_id</th><th>Student Name</th><th>Status</th></tr>";
                        console.log(data);
                        console.log(s1);
                        let temp = data[0][0];
                        for (let x = 0; x < data.length; x++) {
